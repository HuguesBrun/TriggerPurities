...
#if !defined(__CINT__)
ClassImp(TriggerElectrons);
#endif

#define DEBUG
using namespace std;

TriggerElectrons::TriggerElectrons(TTree* tree):
  CMSAnalysisSelectorMiniTrees(tree)  {} 


void TriggerElectrons::Initialise() {
...
void TriggerElectrons::InsideLoop() {
  /*  int nbMuons = T_Muon_Pt->size();
    ...
    if (doDiElectron){
        if (!(T_Event_HLT_Ele17_Ele8==1)) return;
        int nbElec = T_Elec_Pt->size();
        for (int i = 0 ; i < nbElec ; i++){
            for (int j= i+1 ; j < nbElec ; j++){
                if (!((T_Elec_HLT_Ele17TightID_Ele8_Ele17Leg->at(i)==1&&T_Elec_HLT_Ele17TightID_Ele8_Ele8Leg->at(j)==1)||(T_Elec_HLT_Ele17_Ele8_Ele17Leg->at(j)==1&&T_Elec_HLT_Ele17_Ele8_Ele8Leg->at(i)==1))) continue;
                fillTheDiLeptonTree(i,j,0);
...
void TriggerElectrons::fillTheDiLeptonTree(int ite, int ite2, int localType){
    ...
    float deltaPhi = acos(cos(T_Jet_Phi->at(leadJet)-acos(T_Elec_Px->at(ite)/T_Elec_Pt->at(ite))));
    passDeltaPhi = (deltaPhi>1);
    other_pt = T_Elec_Pt->at(ite2);
    other_eta = T_Elec_Eta->at(ite2);
    other_absEta = fabs(other_eta);
    other_passFO = T_Elec_isFO->at(ite2);
    other_passPre = passPreCuts(other_pt, T_Elec_isEB->at(ite2), T_Elec_sigmaIetaIeta->at(ite2), T_Elec_deltaEtaIn->at(ite2), T_Elec_deltaPhiIn->at(ite2) ,T_Elec_HtoE->at(ite2), T_Elec_dr03EcalSumEt->at(ite2), T_Elec_dr03TkSumPt->at(ite2), T_Elec_dr03EcalSumEt->at(ite2));
    other_passIP = passIPcuts(T_Elec_d0->at(ite2), T_Elec_dZ->at(ite2));
    other_passConvs  = passMissItCons(T_Elec_passConversionVeto->at(ite2), T_Elec_nHits->at(ite2));
    other_passTrig0 = T_Elec_HLT_Ele17_Ele8_Ele17Leg->at(ite2);
    other_passTrig1 = T_Elec_HLT_Ele17_Ele8_Ele8Leg->at(ite2);
    
    other_passTrackerIso = (T_Elec_dr03TkSumPt->at(ite2)/other_pt < 0.2);
    other_passHCALIso = (T_Elec_dr03HcalSumEt->at(ite2)/other_pt < 0.2);
    other_passECALIso = (T_Elec_isEB->at(ite2) ? ((T_Elec_dr03EcalSumEt->at(ite2)-1)/other_pt < 0.2) : (T_Elec_dr03EcalSumEt->at(ite2)/other_pt < 0.2));
    other_passSigIeta = (T_Elec_isEB->at(ite2) ? (T_Elec_sigmaIetaIeta->at(ite2)                               < 0.01) : (T_Elec_sigmaIetaIeta->at(ite2)                               < 0.03));
    other_passDeltaEtaSC = (T_Elec_isEB->at(ite2) ? (fabs(T_Elec_deltaEtaIn->at(ite2))        < 0.007) : (fabs(T_Elec_deltaEtaIn->at(ite2))        < 0.009));
    other_passdeltaPhiSC = (T_Elec_isEB->at(ite2) ? (fabs(T_Elec_deltaPhiIn->at(ite2))        < 0.15) : (fabs(T_Elec_deltaPhiIn->at(ite2))        < 0.10));
    other_passHoE = (T_Elec_isEB->at(ite2) ? (T_Elec_HtoE->at(ite2)                              <0.12) : (T_Elec_HtoE->at(ite2)                              < 0.10));
    other_passConversionVtxFit = (T_Elec_passConversionVeto->at(ite2) <= 0);
    other_passMissHit = (T_Elec_nHits->at(ite2) == false);
    other_passd0 =(T_Elec_d0->at(ite2) < 0.02);
    other_passdZ =(T_Elec_dZ->at(ite2) < 0.1);
...
bool TriggerElectrons::passIPcuts(float d0, float dz)
{
...
bool TriggerElectrons::passMissItCons(bool conversion, int nMissHit)
{
...
bool TriggerElectrons::passPreCuts(float myPt, int isEB, float theSigIeta, float deltaEtaSC, float deltaPhiSC,float HOE, float ecalIso, float hcalIso, float trackerIso)
{
